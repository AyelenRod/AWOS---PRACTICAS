    ...
    return (
        <div>
            <ReportHeader
                ...
                description="Distribución de copias físicas por categoría."
            />

            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 mb-8">
                {data.map((cat) => (
                    <div key={cat.category} className="bg-white p-4 rounded-lg border border-slate-200">
                        <div className="flex items-center justify-between mb-2">
                            <h3 className="font-bold text-slate-800">{cat.category}</h3>
                            <Boxes className="text-slate-300 w-5 h-5" />
                        </div>
                        <div className="text-2xl font-bold text-slate-900 mb-4">{cat.total_copies} <span className="text-xs font-normal text-slate-400">copias</span></div>
