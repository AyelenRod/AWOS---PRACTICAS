...
export default async function FinesPage({
    ...
    return (
        <div>
            <ReportHeader
                title="Fines Summary"
                description="Monthly breakdown of fines generated, paid, and pending."
            />

            <div className="bg-white p-4 rounded-md border border-slate-200 mb-6 shadow-sm">
                <form action={filterAction} className="flex gap-4 items-end">
                    ...
                    <div>
                        <label className="block text-xs font-semibold text-slate-500 mb-1">End Month</label>
                    ...
                    {(start || end) && (
                        <a href="/reports/fines" className="text-sm text-slate-500 hover:text-red-600 underline self-center">Reset</a>
                    )}
                </form>
            </div>
                ...
                <table className="w-full text-sm text-left">
                    <thead className="bg-slate-50 text-slate-500 font-medium border-b border-slate-200">
                        <tr>
                            ...
                            <th className="px-4 py-3 text-right">Total Fines Issued</th>
                            <th className="px-4 py-3 text-right">Total Amount</th>
                            <th className="px-4 py-3 text-right text-green-600">Collected</th>
                            <th className="px-4 py-3 text-right text-red-600">Pending</th>
                        </tr>
                    </thead>
                        ...
                        {data.length === 0 && (
                            <tr>
                                <td colSpan={5} className="px-4 py-8 text-center text-slate-500">
                                    No fines record found for this period.
                                </td>
                            </tr>
                        )}
                    </tbody>
