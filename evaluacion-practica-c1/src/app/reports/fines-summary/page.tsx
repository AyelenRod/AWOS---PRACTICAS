...
export default async function FinesSummaryPage() {
    let rows = [];
    try {
        const result = await query('SELECT * FROM vw_fines_summary ORDER BY month_year DESC');
        rows = result.rows;
    } catch (error) {
    ...
    const recentTotal = rows.length > 0 ? `$${rows[0].collected_amount}` : '$0';
        ...
        <main className="min-h-screen bg-gray-50 p-8">
            <div className="max-w-6xl mx-auto">
                ...
                <header className="mb-8">
                    ...
                    <p className="text-gray-600">Estado de recaudación por mes.</p>
                </header>

                <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 mb-8 inline-block min-w-[200px]">
                    <p className="text-sm font-medium text-gray-500 uppercase tracking-wide">
                        Total Recaudado (Mes Reciente)
                    </p>
                    <p className="text-4xl font-bold text-blue-600 mt-2">{recentTotal}</p>
                </div>
